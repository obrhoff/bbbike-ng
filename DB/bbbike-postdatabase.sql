...
UPDATE network SET normal = array_cat(normal, attribute.array) from
(select info.networkid, array_agg(('HA', info.type, st_linemerge(info.relevantGeometry))::attribute) as array from
(select path.networkid, handicap.type ,path.geometry as pathgeometry, st_intersection(handicap.geometry, path.geometry) as relevantGeometry, st_intersection(path.geometry, handicap.geometry) as matchedGeometry from network as path, handicap where st_intersects(handicap.geometry, path.geometry) and not geometryType(st_intersection(path.geometry, handicap.geometry)) = 'POINT') as info where ST_OrderingEquals(info.relevantGeometry, info.matchedGeometry) group by info.networkid ) as attribute where attribute.networkid = network.networkid;

UPDATE network SET reversed = array_cat(reversed, attribute.array) from
(select info.networkid, array_agg(('HA', info.type, st_linemerge(info.relevantGeometry))::attribute) as array from
(select path.networkid, handicap.type ,path.geometry as pathgeometry, st_intersection(handicap.geometry, path.geometry) as relevantGeometry, st_intersection(path.geometry, handicap.geometry) as matchedGeometry from network as path, handicap where st_intersects(handicap.geometry, path.geometry) and not geometryType(st_intersection(path.geometry, handicap.geometry)) = 'POINT') as info where not ST_OrderingEquals(info.relevantGeometry, info.matchedGeometry) group by info.networkid ) as attribute where attribute.networkid = network.networkid;
