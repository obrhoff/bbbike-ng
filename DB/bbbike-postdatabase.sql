...
UPDATE network SET attributes = array_cat(attributes, attribute.array) from (select info.networkid, array_agg(('cyclepath', info.type, info.geometry)::attribute) as array from (select path.networkid, cyclepath.type as type, st_intersection(cyclepath.geometry, path.geometry) as geometry from network as path, cyclepath where st_intersects(cyclepath.geometry, path.geometry) and not geometryType(st_intersection(path.geometry, cyclepath.geometry)) = 'POINT') as info group by info.networkid) as attribute where attribute.networkid = network.networkid;
UPDATE network SET attributes = array_cat(attributes, attribute.array) from (select info.networkid, array_agg(('quality', info.type, info.geometry)::attribute) as array from (select path.networkid, quality.type as type, st_intersection(quality.geometry, path.geometry) as geometry from network as path, quality where st_intersects(quality.geometry, path.geometry) and not geometryType(st_intersection(path.geometry, quality.geometry)) = 'POINT') as info group by info.networkid) as attribute where attribute.networkid = network.networkid;
UPDATE network SET attributes = array_cat(attributes, attribute.array) from (select info.networkid, array_agg(('greenway', info.type, info.geometry)::attribute) as array from (select path.networkid, greenpath.type as type, st_intersection(greenpath.geometry, path.geometry) as geometry from network as path, greenpath where st_intersects(greenpath.geometry, path.geometry) and not geometryType(st_intersection(path.geometry, greenpath.geometry)) = 'POINT') as info group by info.networkid) as attribute where attribute.networkid = network.networkid;
UPDATE network SET attributes = array_cat(attributes, attribute.array) from (select info.networkid, array_agg(('unlit', 'NL', info.geometry)::attribute) as array from (select path.networkid, st_intersection(unlitpath.geometry, path.geometry) as geometry from network as path, unlitpath where st_intersects(unlitpath.geometry, path.geometry) and not geometryType(st_intersection(path.geometry, unlitpath.geometry)) = 'POINT') as info group by info.networkid) as attribute where attribute.networkid = network.networkid;
UPDATE network SET attributes = array_cat(attributes, attribute.array) from (select info.networkid, array_agg(('trafficlight', info.type, info.geometry)::attribute) as array from (select path.networkid, trafficlight.type as type, st_intersection(trafficlight.geometry, path.geometry) as geometry from network as path, trafficlight where st_intersects(trafficlight.geometry, path.geometry) and not trafficlight.type = 'X')  as info group by info.networkid) as attribute where attribute.networkid = network.networkid;